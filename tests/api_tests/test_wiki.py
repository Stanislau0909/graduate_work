...
class TestWiki:
    ...
    def test_create_wiki_without_files(self, access_token_specialist, builder_wiki):
        create_wiki = self.wiki.create_wiki_manual(jwt=access_token_specialist, payload=builder_wiki.build())
        self.wiki.check_status_is_201created(create_wiki)

    @pytest.mark.parametrize("model", [
        '1' * 31
    ])
    def test_create_wiki_invalid_data_field_model(self, access_token_specialist, builder_wiki, model):
        create = self.wiki.create_wiki_manual(jwt=access_token_specialist,
                                              payload=builder_wiki.set_model(model).build())
        self.wiki.check_status_is_422_Unprocessable_Entity(create)
    ...
    def test_create_wiki_invalid_data_field_custom_id(self, access_token_specialist, builder_wiki, custom_id):
        create = self.wiki.create_wiki_manual(jwt=access_token_specialist,
                                              payload=builder_wiki.set_custom_id(custom_id).build())
        self.wiki.check_status_is_422_Unprocessable_Entity(create)
    ...
    def test_adding_attach_in_wiki(self, access_token_admin, get_last_id_wiki_rae, file):
        add_attaches = self.wiki.adding_attach_in_wiki(jwt=access_token_admin,
                                                       get_last_id_wiki_rae=get_last_id_wiki_rae, file=file)
        self.wiki.check_status_is_200ok(add_attaches)
    ...
    def test_delete_attach_in_wiki(self, access_token_admin, get_last_id_wiki_rae, get_id_attaches):
        ...
        self.wiki.check_status_is_200ok(deleted_file)
        update_list_id_files = self.wiki.get_id_attach_files_in_wiki(jwt=access_token_admin)
        assert deleted_file.json()['id'] not in update_list_id_files
    ...
    def test_edit_fields_in_wiki(self, access_token_specialist, get_last_id_wiki_rae, builder_wiki, family,
        ...
        self.wiki.check_status_is_200ok(changed_wiki_from_backend)
        changed_wiki_from_backend_json = changed_wiki_from_backend.json()
        compare_value_general_key = {k: (changed_wiki_from_backend_json.get(k), our_payload.get(k)) for k in
                                     set(changed_wiki_from_backend_json.keys()) & set(our_payload.keys()) if
                                     changed_wiki_from_backend_json.get(k) != our_payload.get(k)}
    ...
    def test_search_field_theme_in_wiki(self, access_token_admin, builder_search_wiki, theme):
        check = self.wiki.check_search_input(jwt=access_token_admin,
                                             payload=builder_search_wiki.set_keyword(theme).build())
        self.wiki.check_status_is_200ok(check)
        self.wiki.check_field_theme_keywords(payload=theme, response=check.json())
        print(theme)
    ...
    def test_download_wiki_pdf(self, access_token_admin, get_last_id_wiki_rae):
        ...
        link_for_download = self.wiki.get_download_link_wiki_azk(access_token_admin=access_token_admin,
                                                                 get_last_id_wiki_rae=get_last_id_wiki_rae)
        self.wiki.check_status_is_200ok(link_for_download)
        paste_link = self.wiki.paste_link_for_download_pdf(
            get_download_link_wiki_azk=link_for_download.json()['link_id'])
        self.wiki.check_status_is_200ok(paste_link)
    ...
    def test_download_wiki_html(self, access_token_admin, get_last_id_wiki_rae):
        ...
        link_for_download = self.wiki.get_download_link_wiki_azk(access_token_admin=access_token_admin,
                                                                 get_last_id_wiki_rae=get_last_id_wiki_rae)
        self.wiki.check_status_is_200ok(link_for_download)
        paste_link = self.wiki.paste_link_for_download_html(
            get_download_link_wiki_azk=link_for_download.json()['link_id'])
        self.wiki.check_status_is_200ok(paste_link)
